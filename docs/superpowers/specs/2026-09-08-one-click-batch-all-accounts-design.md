...
- 日期：2026-09-08
- Task-ID：BAT-001
- 状态：已批准（2026-09-08 Reviewer APPROVED，`reviews/BAT-001-spec-review.md`；按 `.hermes.md` spec 免签，生效即批准）
- 决策来源：2026-09-08 用户需求「目前有两个账号，想在首页添加一个『一键出发』，
  ...
  （串行+错峰）」半，**并行仍是非目标**。
- 评审拍板记录：2026-09-08 用户需求澄清拍板 ×2（①「同时触发」= 串行排队 + 号间强制错峰
  ≥15 分钟，沿用 MAI-001「绝不并发」风控纪律；②各号使用各自已保存文案）。Reviewer 两轮
  评审：第一轮 CHANGES_REQUIRED（P1-F1 等待守卫期陈旧 .running 无自愈 + P2×4）→ spec
  修订（提交 0e85d25）→ 第二轮 APPROVED（P0/P1 无漏网；附 1 条 P2 理论竞态观察，
  处置 = plan 实现约束，不阻塞）。详见 `reviews/BAT-001-spec-review.md`
